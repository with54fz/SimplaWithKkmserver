    ...
    {foreach $purchases as $purchase}
        <tr>
            ...
            <td class="image">
                ...
                {if $image}
                    <a href="products/{$purchase->product->url}"><img src="{$image->filename|resize:50:50}"
...
{if !$order->paid}
            ...
            <ul id="deliveries">
                {foreach $payment_methods as $payment_method}
                    <li>
                        ...
                        <h3><label for=payment_{$payment_method->id}>    {$payment_method->name}, к
                                оплате {$order->total_price|convert:$payment_method->currency_id}
    ...
    {elseif $payment_method}
        <h2>Способ оплаты &mdash; {$payment_method->name}
            <form method=post><input type=submit name='reset_payment_method' value='Выбрать другой способ оплаты'>
            </form>
        </h2>
        <p>
            {$payment_method->description}
...
{* ================ отличие от стандартного шаблона ========================== *}
